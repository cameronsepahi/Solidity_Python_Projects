...
def main():
    ...
    if network.show_active() in ["mainnet-fork"]:
        get_weth()
    # Two things we always need when interacting with a contract: The ABI and the address
    lending_pool = get_lending_pool()
    # Approve sending out ERC20 tokens
    approve_erc20(amount, lending_pool.address, erc20_address, account)
    print("Depositing in lending pool...")
    ...
    # borrowable_eth -> borrowable_dai ( 95%) to be a little cautious so we don't get liquidated
...
def get_asset_price(price_feed_address):
    ...
    # [1] for 'answer' in latestRoundData returns
